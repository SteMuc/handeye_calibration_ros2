...
Description: ROS 2 program for ArUco marker pose estimation based on OpenCV
...
class ArucoNode(Node):
    def __init__(self):
        ...
        if ARUCO_DICT.get(self.aruco_dictionary_name, None) is None:
            self.get_logger().error(f"ArUCo tag of '{self.aruco_dictionary_name}' is not supported")
            return
